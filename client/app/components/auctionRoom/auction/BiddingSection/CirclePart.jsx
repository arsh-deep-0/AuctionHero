    ...
    return (
        <div className="inline-flex items-center justify-center overflow-hidden rounded-full ">
            <svg className=" -rotate-90" style={{width:radius*2+"rem",aspectRatio:"1"}}>
                <circle className="" style={{ color: circleColor }} strokeWidth="0" stroke="currentColor" fill="currentColor" r={radius*16-1.5} cx={radius*16+""} cy={radius*16+""}  />
                <circle className="" style={{ color: progressBarColor }} strokeWidth="3" strokeDasharray={circumference} strokeDashoffset={circumference - (percent / 100) * circumference} strokeLinecap="round" stroke="currentColor" fill="transparent" r={radius*16} cx={radius*16+""} cy={radius*16+""} />
            </svg>
